...
// This is nearly identical to the earlier handout on smart pointers
// but uses a different syntax to "Create" the objects.  Note
// the Ptr constructor is templated to allow up to three arguments
// which are passed to the "T" constructor.  The "Create" methods
// of the prior approach are not needed in this design.
...
public:
  Ptr();
  template <typename T1> Ptr(T1);
  template <typename T1, typename T2> Ptr(T1, T2);
  template <typename T1, typename T2, typename T3> Ptr(T1, T2, T3);
