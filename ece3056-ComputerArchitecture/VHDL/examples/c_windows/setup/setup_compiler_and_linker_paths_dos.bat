...
for /f "delims=" %%v in ('vsim -version') do set vsim_ver=%%v
...
C:\WINDOWS\system32\reg.exe query "HKLM\SOFTWARE\%NodeDir%Microsoft\VisualStudio\9.0\Setup\VC" /v ProductDir > NUL 2>&1
if errorlevel 0 set VCEDITION=VisualStudio
if errorlevel 1 set VCEDITION=VCExpress

for /f "tokens=2* delims=	 " %%a in ('C:\WINDOWS\system32\reg.exe query "HKLM\SOFTWARE\%NodeDir%Microsoft\%VCEDITION%\9.0\Setup\VC" /v ProductDir') do set TOOLPATH="%%b"
if %is64bit%==1 (
