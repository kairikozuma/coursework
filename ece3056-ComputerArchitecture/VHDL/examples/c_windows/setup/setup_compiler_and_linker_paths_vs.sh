...
TOOLPATH=`regtool -q get /HKLM/SOFTWARE/Microsoft/VisualStudio/9.0/Setup/VC/ProductDir`
if [ $? -ne 0 ]; then
	VCEDITION="VCExpress"
else 
...
TOOLPATH=`regtool get /HKLM/SOFTWARE/Microsoft/$VCEDITION/9.0/Setup/VC/ProductDir`
...
if [ ! -f "$TOOLPATH/$cl_path/cl" ] ; then
	echo "ERROR: Couldn't find cl.exe on your machine. Please install Visual Studio 9.0."
	exit 0
else
...
if [ $is64bit -eq 0 ]; then
	MSPDB80=`regtool get /HKLM/SOFTWARE/Microsoft/$VCEDITION/9.0/InstallDir`
	MSPDB80u=`cygpath -u "$MSPDB80"`
	MSPDB80=`cygpath -ms "$MSPDB80"`
	if [ ! -f "$MSPDB80/mspdb80.dll" ] ; then
...
SDKPATH=`regtool get "/HKLM/SOFTWARE/Microsoft/Microsoft SDKs/Windows/CurrentInstallFolder"`
...
if [ $is64bit -eq 1 ]; then
	export LIB="${TOOLPATH}lib/amd64;${SDKPATH}lib/x64"
else
	export LIB="${TOOLPATH}lib;${SDKPATH}lib"
fi
