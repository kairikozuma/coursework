...
foreach TEST {top1 top2 top3} {
	switch $TEST {
		top1 {
			vsim -c top -assertdebug -msgmode both -coverage -novopt -wlf top1.wlf
		}
		top2 {
			vsim -c top -assertdebug -msgmode both -coverage -novopt -wlf top2.wlf +warning
		}
		top3 {
			vsim -c top -assertdebug -msgmode both -coverage -novopt -wlf top3.wlf +warning +error
		}
	}
