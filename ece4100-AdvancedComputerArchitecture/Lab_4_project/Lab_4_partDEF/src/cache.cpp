...
uns64 calc_index(Cache *c, Addr lineaddr) {
  return ~(0xFFFFFFFFFFFFFFFF << c->index_bits) & lineaddr;
}
...
uns cache_find_victim(Cache *c, uns set_index, uns core_id) {
  ...
  /* Static way paritioning */
  } else if (c->policy == 2) {
    /* Get max number of ways allowed for the core */
    uns max_way = (core_id == 0) ? SWP_CORE0_WAYS : c->ways - SWP_CORE0_WAYS;
    ...
    for (uns64 i = 0; i < c->ways; i++) {
      ...
      /* Increment count for each valid way with matching core id */
      if (cache_line->valid && cache_line->core_id == core_id) {
        count_way++;
      }
    }

    bool quota_reached = count_way >= max_way;

    /* If quota reached, LRU within own core_id */
    if (quota_reached) {
      uns64 earliest_access_time = 0xFFFFFFFFFFFFFFFF;

      for (uns64 i = 0; i < c->ways; i++) {
        Cache_Line * cache_line = &(cache_set->cache_line_array[i]);

        /* If earlier access time found with matching core id, use it */
        if ((cache_line->last_access_time < earliest_access_time) && (cache_line->core_id == core_id)) {
          earliest_access_time = cache_line->last_access_time;
          victim_way = i;
        }
      }
    } else { /* Quota not reached, normal LRU replacement */
      /* Track victim line index and earliest start */
      uns64 earliest_access_time = 0xFFFFFFFFFFFFFFFF;
      for (uns64 i = 0; i < c->ways; i++) {
        Cache_Line * cache_line = &(cache_set->cache_line_array[i]);
