 ...
 **********************************************************************/
 ...
 void print_latch_short(Pipeline_Latch* pl) {
  std::cout << "[" << pl->op_id << "] " << pl->tr_entry.op_type << "["
...
void pipe_cycle_ID(Pipeline *p){
  /* Flag for hazards */
  bool* data_flag = (bool *) calloc(sizeof(bool),  PIPE_WIDTH);
  bool* ctrl_flag = (bool *) calloc(sizeof(bool),  PIPE_WIDTH);
  ...
  for(int i=0; i < PIPE_WIDTH; i++){
    ...
    while (jj < PIPE_WIDTH) {
      ...
      /* Remove dependency if earlier instruction has value */
      /*if (ENABLE_EXE_FWD && (id_data_dep || id_ctrl_dep)) {
        Pipeline_Latch * pl = &(p->pipe_latch[ID_LATCH][jj]);
        int kk = 0;
        while (pl->tr_entry.op_type == OP_LD && (id_data_dep || id_ctrl_dep) && kk < PIPE_WIDTH) {
          if (pl->op_id == p->pipe_latch[EX_LATCH][kk].op_id + 1 && check_same_instruction(pl, &(p->pipe_latch[EX_LATCH][kk]))) {
            id_data_dep = false;
            id_ctrl_dep = false;
          }
          kk++;
      ...
      if(!ENABLE_EXE_FWD){
        ...
        /* Only if a load instruction */
        if (p->pipe_latch[EX_LATCH][jj].tr_entry.op_type == OP_LD) {
          ex_ctrl_dep = check_ctrl_dependence(if_pl, &(p->pipe_latch[EX_LATCH][jj]));
          ex_data_dep = check_data_dependence(if_pl, &(p->pipe_latch[EX_LATCH][jj]));
          /* Ignore dependence if previous load instruction loads same value */
          if (ex_ctrl_dep || ex_data_dep) {
            Pipeline_Latch * pl = &(p->pipe_latch[EX_LATCH][jj]);
            int kk = 0;
            while ((ex_data_dep || ex_ctrl_dep) && kk < PIPE_WIDTH) {
