...
images = images(:,1:numPatches);
[h, array] = display_network(images);
locx = (1 + (29 * (0:(floor(size(array, 1)/29)-1)))); 
locy = (1 + (29 * (0:(floor(size(array, 2)/29)-1))));
patches = zeros(visibleSize, numPatches);
%display_network(patches(:,randi(size(patches,2),200,1)),8);

for i=1:length(locx)
    for j=1:length(locy)
        patches(:,i+(j-1)) = reshape(array(locx(i):locx(i)+27,locy(j):locy(j)+27),visibleSize,1);
    end
end
