...
/* Mems microphone */
AnalogIn microphone(p16);

/* Mutex for lcd */
Mutex lcd_mutex; 

/* Sound scale */
volatile double sound_scale = 0.0;
...
void microphone_thread(void const * args) {
    sound_scale = microphone;
}
...
void rgb_thread(void const *args) {
    while (true) {
        ...
        if (sound_scale > 0.21) {
            sound_scale = 1.0;
            rgb_write(r * sound_scale, g * sound_scale, b * sound_scale);
        
            for (double i = sound_scale; i > 0.0; i -= 0.01) {
...
int main() {
    ...
    Thread mic_thr(microphone_thread);
